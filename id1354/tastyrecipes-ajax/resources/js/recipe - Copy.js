...
function getUsername(){
	...
	$.ajax({url: "../Username", async: false, success: function(result){ 
...
function getComments(){	
	$.ajax({url: "../GetCommentsRecipe?recipe="+urlParams.get('recipe'), success: function(result){
		...
		if(comments.length == 0){
			...
			$.each(comments, function (index,comment) {
				...
				if(getUsername() != null && comment.author == getUsername()){
